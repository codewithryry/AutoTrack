...
function ScanResultHeader({ onReset }) {
  return (
    <button
      type="button"
      onClick={onReset}
      className="-ml-1.5 inline-flex items-center gap-1.5 rounded-lg px-1.5 py-1 text-[15px]
...
/**
 * The picture, the name, the id — with the success mark attached to the
 * picture rather than floating above the whole screen.
 *
 * The tick sits on the image corner so "found" reads as a property of *this
 * tool*, which is what the previous version failed at: a banner at the top of
 * a page says the page succeeded, a badge on the photo says the object did.
 */
function ToolIdentity({ tool }) {
  return (
    <div className="flex items-center gap-3.5">
      ...
      <div className="relative h-[96px] w-[96px] shrink-0" data-scan="image">
        ...
        />
        <span
          className="absolute -bottom-1.5 -right-1.5 grid h-[26px] w-[26px] place-items-center
      ...
      <div className="min-w-0 flex-1">
        ...
        {/* Clamped rather than truncated to one line: a name is read here to
            confirm "this is the tool I scanned", and cutting it after a dozen
            characters is often the part that would have done that. Two lines
            bounds the height a genuinely long name can claim — nothing in the
            data model limits how long one may be — without losing the words
            that make it recognisable. */}
        <h2 className="mt-1 line-clamp-2 text-[21px] font-extrabold leading-[1.15] tracking-tight">
          {tool.name}
        </h2>
...
/** One line of state, one line of classification. No row of pills. */
function ToolStatus({ tool, tone, dot }) {
  const detail = [tool.condition, tool.category].filter(Boolean).join(' · ')
  return (
    <div>
      <p className={cx('flex items-center gap-2 text-[15px] font-bold', tone)}>
        <span className={cx('h-2.5 w-2.5 rounded-full', dot)} />
        {tool.status}
      </p>
      {detail && <p className="subtle mt-1 text-[13px]">{detail}</p>}
    </div>
  )
}
 ...
 */
function SecondaryActions({ detailHref, onReport }) {
  ...
  return (
    ...
    >
      <Link to={detailHref} className={row}>
        View tool details
 ...
 * what is this, can I take it, where does it live. History, maintenance
 * records and the full specification are one tap away behind "View tool
 * details" and do not belong in front of somebody standing at a rack.
 *
...
export default function ToolFound({ tool, loan, can, onNavigate, onReset }) {
  ...
  const primary = mayReceive
    ? { label: 'Receive return', to: `/return?tool=${tool.id}` }
    : mayReturnOwn
      ? {
      ...
      : mayIssue
        ? { label: 'Borrow for a student', to: `/borrow?tool=${tool.id}` }
        : mayBorrow
  ...
  return (
    <div className="scan-found space-y-4">
      ...
      <div className="space-y-4">
        ...
        <ToolStatus tool={tool} tone={statusTone} dot={statusDot} />
      ...
      <ReportProblemDialog tool={tool} open={reporting} onClose={() => setReporting(false)} />
    </div>
  )
}
